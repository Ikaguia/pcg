...
void main(){
	vec2 p = vec2(modelPosition.x + 2.0f * modelPosition.z, modelPosition.y + 2.0f * modelPosition.z);
	// vec2 p = (-iResolution.xy + 2.0 * fragCoord.xy) / iResolution.y;

	vec3 ray = normalize(vec3(p, 2.0));
	vec3 pos = worldPosition.xyz / 3.0;

	float s1=noiseSpere(ray,pos,1.0,mat3(modelMatrix),0.5,vec3(0.0),iTime);
	s1=pow(min(1.0,s1*2.4),2.0);
	float s2=noiseSpere(ray,pos,1.0,mat3(modelMatrix),4.0,vec3(83.23,34.34,67.453),iTime);
	s2=min(1.0,s2*2.2);
	...
	float s3=ringRayNoise(ray,pos,0.96,1.0,mat3(modelMatrix),iTime);
	fragColor.xyz += mix(vec3(1.0,0.6,0.1),vec3(1.0,0.95,1.0),pow(s3,3.0))*s3;

	float zero=sphereZero(ray,pos,0.9);
	if(zero>0.0){
		vec4 s4=noiseSpace(ray,pos,100.0,mat3(modelMatrix),0.05,vec3(1.0,2.0,4.0),0.0);
		s4.x=pow(s4.x,3.0);
