...
float fbm6(vec2 p){
    float f = 0.0;
    f += 0.500000*(0.5+0.5*noise( p )); p = m*p*2.05;
    f += 0.250000*(0.5+0.5*noise( p )); p = m*p*2.04;
    f += 0.125000*(0.5+0.5*noise( p )); p = m*p*2.03;
    f += 0.062500*(0.5+0.5*noise( p )); p = m*p*2.02;
...
void main(){
    // vec2 p = (-iResolution.xy+2.0*fragTex)/iResolution.y;

    outputColor = vec4( doMagic( fragTex*2 ), 1.0 );
}
